...
export default function ForcePasswordChange({ reason }: { reason: 'provisoria' | 'recuperacao' }) {
  const { profile, reloadProfile, clearRecovery, signOut } = useAuth()
  ...
  const titulo = reason === 'recuperacao' ? 'Redefinir senha' : 'Crie sua nova senha'
  const subtitulo = reason === 'recuperacao'
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      // Limpa o flag de senha provisória conforme o perfil.
      if (reason === 'provisoria') {
        if (profile?.kind === 'patient' && profile.patient) {
          await updatePatient(profile.patient.id, { senha_provisoria: false })
        } else if (profile?.kind === 'staff' && profile.professional) {
          await supabase.from('professionals').update({ senha_provisoria: false }).eq('id', profile.professional.id)
        }
      }
      clearRecovery()
      await reloadProfile()
      ...
      setErro(
        m.includes('different from the old') ? 'A nova senha deve ser diferente da atual.'
          : m.includes('weak') || m.includes('pwned') || m.includes('leaked') ? 'Senha muito fraca ou comprometida. Escolha outra.'
            : m.includes('length') || m.includes('at least') || m.includes('6 char') ? 'A senha é muito curta.'
    ...
    <div className="min-h-full flex items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-2xl border border-black/5 bg-white p-8 shadow-sm">
        <div className="mb-5 text-center">
          ...
          <h1 className="mt-3 text-lg font-semibold text-texto">{titulo}</h1>
          <p className="text-sm text-texto/60">{subtitulo}</p>
        </div>

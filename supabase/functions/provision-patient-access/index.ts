...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: pat } = await admin.from('patients').select('id, email, cpf').eq('id', patient_id).single()
    ...
    // 3) Cria o usuário (confirmado) com a senha provisória.
    ...
    // 4) Vincula ao cadastro e marca senha provisória.
    await admin
      .from('patients')
      .update({ auth_user_id: created.user.id, senha_provisoria: true })
      .eq('id', patient_id)

    return json({ ok: true, login: loginEmail })
  } catch (e) {

# Gera docs/MIGRACAO_PRODUCAO.pdf a partir de docs/MIGRACAO_PRODUCAO.md
...
MD = os.path.join(HERE, 'MIGRACAO_PRODUCAO.md')
OUT = os.path.join(HERE, 'MIGRACAO_PRODUCAO.pdf')
...
story.append(Paragraph('Guia de Migração para Produção', S['title']))
story.append(Spacer(1, 3*mm))
story.append(Paragraph('Instituto Geny Freitas — Passo a passo seguro (para leigos)', S['sub']))
story.append(Spacer(1, 2*mm))
story.append(Paragraph('Supabase &middot; Vercel &middot; Claude &middot; GitHub · Junho/2026', S['sub']))
story.append(Spacer(1, 2*mm))
story.append(Paragraph('De defy-clinicas.vercel.app para institutogenyfreitas.vercel.app', S['sub']))
...
while i < n:
    ...
    if line.lstrip().startswith('- ') or line.lstrip().startswith('- [ ]'):
        txt = line.lstrip()[2:]
        story.append(Paragraph(inline(txt), S['bullet'], bulletText='–')); i += 1; continue
...
def footer(canvas, doc):
    ...
    canvas.drawString(18*mm, 12*mm, 'Instituto Geny Freitas — Guia de Migração para Produção')
...
doc = SimpleDocTemplate(OUT, pagesize=A4, leftMargin=18*mm, rightMargin=18*mm, topMargin=16*mm, bottomMargin=18*mm,
                        title='Guia de Migração para Produção — Instituto Geny Freitas')
